...
char *insert_line(Buffer *buf, char *line, size_t index) {
  ...
  char **newcontents =
      (char **)realloc(buf->contents, (buf->linecount + 1) * sizeof(char *));
...
char *remove_line(Buffer *buf, int buf_row) {
  ...
  char **newcontents =
      realloc(buf->contents, (buf->linecount) * sizeof(char *));
...
void drawbuffer(short starty, short startx, int win_height, int win_width,
                Buffer *buffer, const short colors[]) {
  ...
  for (int row = (int)buffer->ypos; row < buffer->ypos + win_height; row++) {
    ...
    if (row >= buffer->linecount) {
      ...
      highlighted_line =
          highlightsubstr(buffer->contents[row], buffer->xpos, end - 1, colors);
...
void update_statusbar(char *words, short ypos, short width, Buffer *buf,
                      const short colors[], size_t cy, size_t cx) {
